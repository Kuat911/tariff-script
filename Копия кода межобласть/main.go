...
const (
	...
	messageType  = "INTRA_REPUBLICAN"
	weight       = 20
	isCoverWagon = false
)

var excelFiles = []string{"Крытые Межобласть.xlsx", "Зерновозы межобласть.xlsx", "Полувагоны_и_Платформы_межобласть.xlsx", "Полувагоны_и_Платформы_межобласть_измененный_семена_подсолнечника.xlsx"}
...
func sendGetRequest(client *resty.Client, distance int, cargoCode, fileName string, logFile *os.File, requestCount int) (ApiResponse, error) {
	wagonKindId := 57
	switch fileName {
	case "Крытые Межобласть.xlsx":
		wagonKindId = 57
	case "Зерновозы межобласть.xlsx":
		wagonKindId = 43
	case "Полувагоны_и_Платформы_межобласть.xlsx":
		wagonKindId = 63

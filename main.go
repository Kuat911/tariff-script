...
var excelFiles = []string{"Платформы Транзит.xlsx", "Полувагоны Транзит.xlsx", "Крытые Транзит.xlsx"}
...
func sendGetRequest(client *resty.Client, distance int, cargoCode, fileName string, logFile *os.File, requestCount int) (ApiResponse, error) {
	wagonKindId := 57
	switch fileName {
	case "Крытые Транзит.xlsx":
		wagonKindId = 57
	case "Платформы Транзит.xlsx":
		wagonKindId = 63
	case "Полувагоны Транзит.xlsx":
		wagonKindId = 75
